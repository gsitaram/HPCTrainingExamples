...
In this version of the Ghost Exchange, we port the initial code to GPUs using HIP.
This uses the managed memory model, so the only differences between the original code 
and this version are the addition of HIP kernels instead of CPU compute loops.
